...
- 修复 RecallLoom `seed_initial_continuity` 写入路径：本地 RecallLoom dispatcher 的 current-state 刷新逻辑已最小调整为同时接受 `seed_initial_continuity` 与 `update_rolling_summary`，随后通过 dispatcher 写入 `rolling_summary.md`、追加 2026-07-28 daily log，并完成 post-append summary sync。
...
- FSD50K 真实类别级 AP/F1 和阈值敏感性分析已完成，但仍只基于验证集；尚未做测试集提交、类别专属阈值校准或长尾类别重采样实验。
...
- RecallLoom 本地安装包已做最小热修复；后续若执行 `npx skills update` 覆盖该包，需要确认上游是否已包含 `seed_initial_continuity` 放行逻辑，否则可能再次出现 current-state 初始写入被阻止的问题。
