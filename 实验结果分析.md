...
后续可以从三个方向继续完善。第一，结合真实类别名进一步分析 `helicopter`、`pig`、`door_wood_creaks`、`airplane` 等 AST 相对较弱类别的错误来源。第二，如果时间允许，运行更多 fold 或调整 AST 训练轮数、学习率、冻结策略，以验证 0.9300 Accuracy 是否稳定。第三，FSD50K 多标签 AST 扩展已经完成一轮初步训练，最佳验证 mAP 为 0.6208；后续应继续分析类别级 mAP、阈值敏感性和长尾类别表现。
