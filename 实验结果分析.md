...
当前实验仍有一些限制。首先，ESC-50 结果基于单一 fold 划分，虽然 ESC-50 官方 fold 保证了基本可比性，但如果要得到更稳健结论，仍应进行 5-fold 交叉验证。其次，AST 微调虽然效果强，但训练集 Accuracy 最后一轮达到 1.0000，说明模型容量较大，仍需要关注小数据集过拟合问题。最后，FSD50K 扩展实验目前只完成 5 epoch 初步微调和验证集分析，尚未进行更长训练、多阈值模型选择或测试集提交，因此多标签结论仍应视为扩展验证结果。

## 9. 后续工作

后续可以从三个方向继续完善。第一，结合真实类别名进一步分析 `helicopter`、`pig`、`door_wood_creaks`、`airplane` 等 ESC-50 AST 相对较弱类别的错误来源。第二，如果时间允许，运行更多 fold 或调整 AST 训练轮数、学习率、冻结策略，以验证 0.9300 Accuracy 是否稳定。第三，FSD50K 多标签 AST 扩展已经完成类别级和阈值敏感性分析：AP 较高的类别包括 `Burping_and_eructation`、`Cat`、`Thunder`、`Thunderstorm` 和 `Toilet_flush`，AP 较低的类别包括 `Tick`、`Screech`、`Wood`、`Chatter` 和 `Rattle`；阈值扫描显示 micro-F1 最优阈值为 0.20，macro-F1 最优阈值为 0.15，说明固定 0.50 阈值偏保守，后续可进一步做阈值校准和长尾类别优化。
