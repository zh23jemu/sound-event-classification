...
AST 的混淆矩阵呈现清晰对角线，大部分类别都能被正确识别。类别级结果显示，即使表现较弱的类别也明显优于 CNN 实验，例如当前 AST 摘要中最弱类别 `class_40` 的 Accuracy 为 0.5000，其余弱类如 `class_02`、`class_33`、`class_47` 分别达到 0.6250、0.7500 和 0.7500。相比之下，CNN 和 SpecAugment 实验中存在多个 Accuracy 为 0 的类别。

![Pretrained AST confusion matrix](outputs/esc50_ast/analysis/confusion_matrix_normalized.png)

需要注意的是，本地结果分析时没有同步 ESC-50 的 `meta/esc50.csv`，因此类别级图表暂时显示为 `class_00` 到 `class_49`。如果报告需要展示真实类别名称，可以在服务器或同步元数据后重新运行分析脚本。
...
当前实验仍有一些限制。首先，结果基于单一 fold 划分，虽然 ESC-50 官方 fold 保证了基本可比性，但如果要得到更稳健结论，仍应进行 5-fold 交叉验证。其次，类别级结果目前使用数字类别编号，本地没有真实类别名称，不利于详细错误分析。第三，AST 微调虽然效果强，但训练集 Accuracy 最后一轮达到 1.0000，说明模型容量较大，仍需要关注小数据集过拟合问题。最后，实验尚未覆盖 FSD50K 或多标签任务，因此结论主要适用于 ESC-50 单标签分类。

## 9. 后续工作

后续可以从三个方向继续完善。第一，补充 ESC-50 元数据并重新生成带真实类别名的混淆矩阵，便于在报告中解释错误类别。第二，如果时间允许，运行更多 fold 或调整 AST 训练轮数、学习率、冻结策略，以验证 0.9300 Accuracy 是否稳定。第三，在最终报告中将 FSD50K、多标签分类和音视频迁移作为未来工作，而不是继续扩大当前工程范围。
