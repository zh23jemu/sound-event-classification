...
后续可以从三个方向继续完善。第一，结合真实类别名进一步分析 `helicopter`、`pig`、`door_wood_creaks`、`airplane` 等 AST 相对较弱类别的错误来源。第二，如果时间允许，运行更多 fold 或调整 AST 训练轮数、学习率、冻结策略，以验证 0.9300 Accuracy 是否稳定。第三，项目已明确扩展 FSD50K，因此下一阶段应进入多标签 AST 实验，并使用 mAP、micro-F1 和 macro-F1 评价模型表现。
