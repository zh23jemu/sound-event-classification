...
后续工作可以从三个方向展开。第一，围绕 `helicopter`、`pig`、`door_wood_creaks`、`airplane` 等相对较弱类别补充更细的错误案例分析。第二，对 AST 进行轻量调参，例如减少训练轮数、加入早停、调整学习率或冻结部分层，以验证 0.9300 Accuracy 的稳定性。第三，在时间和算力允许时，将流程扩展到 FSD50K 子集或多标签分类任务，并将音视频预训练迁移作为未来研究方向。
