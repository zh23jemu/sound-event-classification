...
声音事件分类旨在从环境音频中识别具有语义意义的声音类别，是计算机听觉和多媒体理解中的基础任务。本项目围绕 ESC-50 环境声音分类数据集，构建了从 Log-Mel Spectrogram、CNN baseline、SpecAugment 数据增强到预训练 Audio Spectrogram Transformer（AST）微调的递进实验流程，并进一步扩展到 FSD50K 多标签声音事件分类。ESC-50 实验采用官方 fold 划分，将 fold 1-4 作为训练集、fold 5 作为验证集。结果显示，从头训练的 CNN baseline 最佳验证 Accuracy 为 0.4125，加入 SpecAugment 后提升至 0.4200，提升幅度较小；使用 AudioSet 预训练 AST 微调后，最佳验证 Accuracy 达到 0.9300，相比 CNN baseline 提升 +0.5175。FSD50K 扩展实验中，预训练 AST 在 200 类多标签任务上取得最佳验证 mAP 0.6208；阈值敏感性分析显示，将决策阈值从 0.50 调整到 0.20 可将 micro-F1 提升到 0.7101。实验表明，大规模音频预训练 Transformer 相比从头训练的小型 CNN 具有更强的迁移能力和扩展性。项目最终形成了可复现训练代码、Slurm 脚本、训练曲线、混淆矩阵、类别级指标、多标签指标、模型权重和实验日志。
...
本项目的研究问题是：在课程项目可用时间和算力条件下，如何利用 Log-Mel Spectrogram 与预训练 Transformer 模型，构建一个可复现、可评估、具有较好泛化能力的声音事件分类系统？围绕该问题，项目设计了三组递进实验：CNN baseline、CNN + SpecAugment、Pretrained AST fine-tuning。通过这些实验，可以比较从头训练小模型、频谱增强策略和预训练 Transformer 迁移之间的差异。项目的主要贡献在于：第一，完成 ESC-50 单标签分类的训练、评估和可视化闭环；第二，证明预训练 AST 相比轻量 CNN baseline 具有显著优势；第三，将同一主线扩展到 FSD50K 多标签任务，并补充类别级 AP/F1 与阈值敏感性分析，使结论不只停留在整体指标层面。
...
训练和分析代码均已保存到项目仓库。CNN 与 SpecAugment 实验可使用 `scripts/train.py` 运行，ESC-50 AST 实验使用 `scripts/train_ast.py`，FSD50K 多标签实验使用 `scripts/train_fsd50k_ast.py`。实验结果通过 `scripts/analyze_esc50_results.py`、`scripts/compare_esc50_experiments.py` 和 `scripts/analyze_fsd50k_results.py` 生成训练曲线、混淆矩阵、类别级指标、阈值敏感性图和摘要。
...
实验结果支持项目的核心假设：在小规模声音事件分类任务中，预训练音频 Transformer 的迁移学习效果显著优于从头训练的小型 CNN。CNN baseline 的价值在于验证流程可行，并提供可复现对照；SpecAugment 提供了训练策略比较，但当前参数下提升有限；AST 则构成项目最重要的模型结果。换言之，本项目的有效提升主要来自大规模音频预训练表征，而不是单纯依赖小模型结构或轻量数据增强。
...
本项目使用 AI 工具辅助完成文献梳理、项目计划整理、代码框架生成、训练脚本调试、实验日志记录和报告初稿撰写。实验指标、训练结果和结论均来自实际运行的代码、服务器输出和同步回本地的分析文件；AI 工具主要承担整理、生成草稿和辅助排查问题的作用，最终技术判断仍以原始论文、项目代码、实验日志和实际指标为依据。
